...
class UserController extends Controller
{
    ...
    public function signUp(Request $request)
    {
        ...
        $smsby = DB::table('smsby')
               ->first();
            //    checks smsby status and firebase status
            //→ Means you are using SMS OTP system, not Firebase.
        if ($smsby->status == 1 && $firebase->status == 0)
        {
            // check for otp verify
            // If user already verified → return status = 0.
            ...
            // If user not verified → insert/update user, generate OTP, send via otpmsg(), and return:
        ...
        //Means you are using Firebase OTP system.
        elseif ($firebase->status == 1)
        {
            ...
            // If new user → insert into DB with is_verified = 0, no OTP generated here (Firebase will handle OTP)
             else {
                if ($request->user_image) {
                    $user_image = $request->user_image;
                    $user_image = str_replace('data:image/png;base64,', '', $user_image);
        ...
        // Else (when both SMS and Firebase are disabled or not set properly)
        // → User is inserted with is_verified = 1 directly, no OTP required.
        else {
            if ($checkUser && $checkUser->is_verified == 1) {
                $message = array('status' => '0', 'message' => 'user already registered');
                return $message;
